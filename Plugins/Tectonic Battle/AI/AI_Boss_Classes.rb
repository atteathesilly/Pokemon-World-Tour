    ...
    def initialize(user, battle)
        ...
        @beforePhaseChange.push(proc { |user, _battle|
            if user.avatarPhase == 0
                formChangeMessage = _INTL("The boss Deoxys turns to Attack Form!")
                user.pbChangeForm(1, formChangeMessage)
            elsif user.avatarPhase == 1
                formChangeMessage = _INTL("The boss Deoxys turns to Defense Form!")
...
def calyrexArrives(user, battle, version = 0)
    ...
    battle.pbDisplayBossNarration(_INTL("The boss Calyrex has arrived to save its steed!"))
...
class PokeBattle_AI_GENESECT < PokeBattle_AI_Boss
    def initialize(user, battle)
        ...
        @beginBattle.push(proc { |user, battle|
            battle.pbDisplayBossNarration(_INTL("The boss Genesect is analyzing your whole team for weaknesses..."))
            weakToElectric	= 0
            weakToFire	= 0
            weakToIce	= 0
            weakToWater	= 0
