...
def reviveMixFossils(fossil1,fossil2)
	...
	fossilsToSpecies = {
		...
		%i[FOSSILIZEDSTEM FOSSILIZEDPUMPKIN] => :CLTSAREENA,
		%i[FOSSILIZEDSTEM FOSSILIZEDTHING] => :TSARLETT,
		%i[FOSSILIZEDROCK FOSSILIZEDTHING] => :CLDIGLETT,
		%i[FOSSILIZEDROCK FOSSILIZEDPUMPKIN] => :DIGLEGGS,
	...
	if chosenSpecies.nil?
		pbMessage(_INTL("Galarian Fossils and Callie fossils don't go together. Trust me, I've tried."))
		return
	end
...
def pbChooseMixFossilHead(var = 0)
	...
	pbFadeOutIn {
	  ...
	  ret = screen.pbChooseItemScreen(Proc.new { |item| %i[FOSSILIZEDBIRD FOSSILIZEDFISH FOSSILIZEDSTEM FOSSILIZEDROCK].include?(item) })
	}
...
def pbChooseMixFossilBody(var = 0)
	...
	pbFadeOutIn {
	  scene = PokemonBag_Scene.new
	  screen = PokemonBagScreen.new(scene,$PokemonBag)
	  ret = screen.pbChooseItemScreen(Proc.new { |item| %i[FOSSILIZEDDRAKE FOSSILIZEDDINO FOSSILIZEDPUMPKIN FOSSILIZEDTHING].include?(item) })
	}
	$game_variables[var] = ret || :NONE if var > 0
	return ret
end
...
def styleRubyAlcremie
    ...
    return false if pbGet(1) < 0
    pkmn = $Trainer.party[pbGet(1)]
    possibleForms, possibleFormNames = getFormSelectionChoices(:ALCREMIE,pkmn.form,allow = [7,8,9,10,11,12,13])
    pbMessage(_INTL("What flavor would you like me to give it?"))
    choice = pbShowCommands(nil,possibleFormNames,possibleFormNames.length+1)
    if choice < possibleForms.length
        pbMessage(_INTL("{1} remixed to {2}!", pkmn.name, possibleFormNames[choice]))
...
def styleMatchaAlcremie
    ...
    return false if pbGet(1) < 0
    pkmn = $Trainer.party[pbGet(1)]
    possibleForms, possibleFormNames = getFormSelectionChoices(:ALCREMIE,pkmn.form,allow = [14,15,16,17,18,19,20])
    pbMessage(_INTL("What flavor would you like me to give it?"))
    choice = pbShowCommands(nil,possibleFormNames,possibleFormNames.length+1)
    if choice < possibleForms.length
        pbMessage(_INTL("{1} remixed to {2}!", pkmn.name, possibleFormNames[choice]))
...
def styleLemonAlcremie
    ...
    return false if pbGet(1) < 0
    pkmn = $Trainer.party[pbGet(1)]
    possibleForms, possibleFormNames = getFormSelectionChoices(:ALCREMIE,pkmn.form,allow = [28,29,30,31,32,33,34])
    pbMessage(_INTL("What flavor would you like me to give it?"))
    choice = pbShowCommands(nil,possibleFormNames,possibleFormNames.length+1)
    if choice < possibleForms.length
        pbMessage(_INTL("{1} remixed to {2}!", pkmn.name, possibleFormNames[choice]))
